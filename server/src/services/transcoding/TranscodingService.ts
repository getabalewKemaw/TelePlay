...
import { createFFmpegExecutor } from '../ffmpeg/implementations/FFmpegExecutor.js';
...
export const createTranscodingService = (
  ffmpegExecutor: IFfmpegExecutor = createFFmpegExecutor()
): ITranscodingService => {
  ...
  const performTranscoding = async (
    ...
    await ffmpegExecutor.execute(commandOptions);
  };

  const transcodeChunk = async (params: ChunkTranscodingParams): Promise<TranscodingResult> => {
  ...
  const getRecommendedTargetCodec = (sourceCodec: string): string => {
...
export type TranscodingService = ReturnType<typeof createTranscodingService>;
