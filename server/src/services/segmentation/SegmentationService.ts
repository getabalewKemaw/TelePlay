...
import { createSegmentationStrategyFactory } from './strategies/SegmentationStrategyFactory.js';
...
export const createSegmentationService = (
  ...
  const segmentationStrategyFactory = createSegmentationStrategyFactory();
  ...
  const getAllSegments = async (filePath: string, options?: SegmentationOptions): Promise<SegmentMetadata[]> => {
    if (!filePath) {
    ...
    const chunks = await chunkingService.getAllChunks(filePath, {
  ...
  const getSegmentsInRange = async (
    filePath: string,
    startTime: number,
    endTime: number,
...
export type SegmentationService = ReturnType<typeof createSegmentationService>;
